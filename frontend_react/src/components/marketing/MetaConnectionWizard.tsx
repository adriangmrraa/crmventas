...
import { CheckCircle2, ChevronRight, Briefcase, BarChart3, Loader2, Zap, Building2 } from 'lucide-react';
...
export default function MetaConnectionWizard({ isOpen, onClose, onSuccess }: MetaConnectionWizardProps) {
    ...
    const loadPortfolios = async () => {
        ...
        try {
            ...
            const bms: any[] = data?.data || [];
            setPortfolios(bms);

            // Auto-skip if only 1 BM — go directly to accounts
            if (bms.length === 1) {
                setSelectedPortfolio(bms[0]);
                await loadAccounts(bms[0].id);
            }
        } catch (err: any) {
            setError(err.response?.data?.detail || 'Error cargando Business Managers. Asegurate de haber conectado Meta primero.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white rounded-[28px] w-full max-w-lg overflow-hidden shadow-2xl my-auto">
                ...
                <div className="p-7 border-b border-gray-100 bg-gradient-to-br from-blue-600 to-blue-700">
                    ...
                    {/* Step progress (only show for steps 1-2) */}
                    {step < 3 && (
                ...
                <div className="p-7">
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 gap-4">
                            ...
                            <p className="text-gray-500 font-medium">Cargando datos desde Meta...</p>
                        </div>
                    ) : (
                        <>
                            ...
                            {step === 1 && (
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Paso 1 de 2</p>
                                        <h3 className="text-lg font-bold text-gray-900">Selecciona el Business Manager</h3>
                                        <p className="text-sm text-gray-500 mt-1">Elige el portafolio de Meta que contiene tu cuenta de anuncios.</p>
                                    </div>
                            ...
                            {step === 2 && (
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Paso 2 de 2</p>
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        <button
                                            onClick={() => { setStep(1); setSelectedAccount(null); }}
