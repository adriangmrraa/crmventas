...
export const BulkStatusUpdate: React.FC<BulkStatusUpdateProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-[#0d1117] rounded-2xl shadow-xl shadow-black/20 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">

                {/* Header */}
                <div className="px-6 py-4 border-b border-white/[0.04] flex justify-between items-center bg-white/[0.03]/[0.02]">
                    <div className="flex items-center gap-2 text-white font-semibold">
                        <Layers className="w-5 h-5 text-blue-400" />
                        {t('leads.bulk_update.title')}
                    </div>
                    <span className="bg-blue-500/100/10 text-blue-400 py-0.5 px-2.5 rounded-full text-xs font-bold">
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    <div className="space-y-5">
                        ...
                        {/* Disclaimer */}
                        <div className="bg-blue-500/100/10 text-blue-400 p-3 rounded-xl flex gap-3 text-sm border border-blue-500/20 items-start">
                        ...
                        <div>
                            ...
                            {isLoadingStatuses ? (
                                ...
                                <select
                                    className="w-full text-sm border-white/[0.06] rounded-xl focus:ring-blue-500 focus:border-blue-500 p-2.5 bg-white/[0.03]/[0.04] text-white hover:bg-white/[0.06]/[0.03]/[0.06] transition-colors cursor-pointer"
                                    value={targetStatus}
                        ...
                        <div>
                            ...
                            <textarea
                                className="w-full text-sm border-white/[0.06] rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 bg-white/[0.03]/[0.04] text-white hover:bg-white/[0.06]/[0.03]/[0.06] transition-colors custom-scrollbar resize-none"
                                rows={3}
                    ...
                    <div className="mt-8 flex justify-end gap-3">
                        <button
                            ...
                            onClick={onCancel}
                            disabled={isUpdating}
                            className="px-4 py-2 text-sm font-semibold text-white/50 hover:text-white hover:bg-white/[0.06]/[0.03]/[0.06] rounded-xl transition-colors disabled:opacity-50"
                        >
                            {t('common.cancel')}
                        </button>
                        <button
                            ...
                            className="px-5 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all disabled:opacity-50 flex items-center"
                        >
                            {isUpdating ? (
