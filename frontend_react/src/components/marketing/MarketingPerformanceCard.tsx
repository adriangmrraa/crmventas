...
export default function MarketingPerformanceCard({ stats: externalStats, loading: externalLoading, timeRange = 'last_30d' }: MarketingPerformanceCardProps) {
    ...
    if (loading) return (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-3xl p-8 h-full flex items-center justify-center">
    ...
    return (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-3xl p-8 h-full flex flex-col justify-between overflow-hidden relative">
            <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50 rounded-full blur-3xl -mr-32 -mt-32 opacity-50"></div>

            <div className="relative z-10">
                <div className="flex justify-between items-start mb-8">
                    <div>
                        <h3 className="text-white/40 font-medium text-xs sm:text-sm mb-1 uppercase tracking-wider">{t('marketing.roi_card_title')}</h3>
                        <p className="text-3xl sm:text-4xl font-black text-white leading-tight">
                            {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
                        </p>
                    </div>
                    <div className={`p-2 sm:p-3 rounded-2xl ${roi >= 0 ? 'bg-green-500/10 text-green-400' : 'bg-rose-100 text-rose-600'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 mb-8">
                    <div className="space-y-1">
                        <div className="flex items-center gap-1.5 text-white/40 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
                            <DollarSign size={14} className="text-indigo-500" /> {t('marketing.investment')}
                        </div>
                        <p className="text-xl sm:text-2xl font-bold text-white break-words">{stats?.currency === 'USD' ? '$' : stats?.currency || ''}{investment.toLocaleString()}</p>
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-1.5 text-white/40 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
                            <Target size={14} className="text-emerald-500" /> {t('marketing.return')}
                        </div>
                        <p className="text-xl sm:text-2xl font-bold text-white break-words">{stats?.currency === 'USD' ? '$' : stats?.currency || ''}{revenue.toLocaleString()}</p>
                    </div>
                </div>
            </div>

            <div className="relative z-10 grid grid-cols-3 gap-1 sm:gap-4 pt-6 border-t border-white/[0.04]">
                ...
                <div className="text-center">
                    <p className="text-white/30 text-[9px] sm:text-[10px] font-bold uppercase mb-1 truncate px-1">{t('marketing.leads')}</p>
                    <p className="text-[10px] sm:text-sm font-bold text-white/70">{stats?.patients_converted || 0}</p>
                </div>
            </div>
        </div>
    );
