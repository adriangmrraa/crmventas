...
export default function ProfessionalsView() {
  ...
  return (
    <div className="flex flex-col h-full bg-white/[0.03]/[0.04] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="bg-white/[0.03] rounded-xl  p-4 border-l-4 border-primary">
            <div className="text-xs text-white/40 uppercase font-bold tracking-tight">{t('professionals.total_staff')}</div>
            <div className="text-3xl font-black mt-1 text-white">{professionals.length}</div>
          </div>
          <div className="bg-white/[0.03] rounded-xl  p-4 border-l-4 border-green-500">
            <div className="text-xs text-white/40 uppercase font-bold tracking-tight">{t('professionals.active_doctors')}</div>
            <div className="text-3xl font-black mt-1 text-green-400">{getActiveProfessionals()}</div>
          </div>
          <div className="bg-white/[0.03] rounded-xl  p-4 border-l-4 border-white/[0.06] sm:col-span-2 lg:col-span-1">
            <div className="text-xs text-white/40 uppercase font-bold tracking-tight">{t('professionals.on_pause')}</div>
            <div className="text-3xl font-black mt-1 text-white/30">
        ...
        {loading ? (
          <div className="flex items-center justify-center p-12 bg-white/[0.03] rounded-2xl border border-white/[0.04] ">
            <div className="flex flex-col items-center gap-3">
              <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
              <p className="text-white/40 font-medium">{t('professionals.loading_team')}</p>
            </div>
          </div>
        ) : professionals.length === 0 ? (
          <div className="p-12 text-center bg-white/[0.03] rounded-2xl border border-white/[0.04] ">
            <div className="mx-auto w-16 h-16 bg-white/[0.03]/[0.02] rounded-full flex items-center justify-center text-white/20 mb-4">
              <ClipboardList size={32} />
            </div>
            <h3 className="text-lg font-bold text-white">{t('professionals.no_professionals')}</h3>
            <p className="text-white/40 max-w-xs mx-auto text-sm mb-6">{t('professionals.empty_hint')}</p>
            <button
              onClick={openCreateModal}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {professionals.map((professional: Professional) => (
              <div
                key={professional.id}
                className="bg-white/[0.03] rounded-2xl border border-white/[0.06]  hover:shadow-md transition-all overflow-hidden flex flex-col group"
              >
                {/* Card Header Profile */}
                <div className="p-5 flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-3 sm:hidden">
                      {professional.email && (
                        <div>
                          <span className="block text-[9px] font-black text-white/30 uppercase tracking-tight mb-0.5">{t('professionals.email')}</span>
                          <div className="flex items-center gap-1.5 text-xs text-white/50 truncate font-semibold">
                      ...
                      {professional.phone && (
                        <div>
                          <span className="block text-[9px] font-black text-white/30 uppercase tracking-tight mb-0.5">{t('professionals.whatsapp')}</span>
                    ...
                    <div className="hidden sm:space-y-1 sm:mt-2 sm:block">
                      {professional.email && (
                        <div className="flex items-center gap-2 text-[11px] text-white/40 truncate">
                          <Mail size={12} className="text-white/30" /> {professional.email}
                        </div>
                      )}
                      {professional.phone && (
                        <div className="flex items-center gap-2 text-[11px] text-white/40">
                          <Phone size={12} className="text-white/30" /> {professional.phone}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Card Divider Text */}
                <div className="px-5 py-3 border-t border-white/[0.04] flex items-center justify-between bg-white/[0.03]/[0.02]/30">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-3 text-white/40 hover:text-primary hover:bg-white/[0.03] rounded-xl transition-all border border-transparent hover:border-white/[0.06] min-h-[44px] min-w-[44px] flex items-center justify-center"
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl transition-all border border-transparent min-h-[44px] min-w-[44px] flex items-center justify-center ${professional.is_active
                        ...
                        : 'text-white/30 hover:text-green-400 hover:bg-green-500/10 hover:border-green-500/20'
                        }`}
                ...
                {professional.working_hours && getTotalSlots(professional.working_hours) > 0 && (
                  <div className="px-5 pb-5">
                    <button
                      ...
                      className="w-full flex items-center justify-between p-2.5 bg-white/[0.03]/[0.02] rounded-xl text-xs font-bold text-white/50 hover:bg-white/[0.03]/[0.04] transition-colors"
                    >
                      ...
                      <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                        {DAYS.map(day => {
                          ...
                          if (config?.enabled && config.slots.length > 0) {
                            return (
                              <div key={day.key} className="flex items-start justify-between text-[11px] border-b border-white/[0.04] pb-1.5 last:border-0 last:pb-0">
                                <span className="font-bold text-white/70">{t('approvals.day_' + day.key)}</span>
                                <div className="text-right text-white/40 font-medium">
                                  {config.slots.map((s, idx) => (
                                    <div key={idx} className="bg-white/[0.03] px-1.5 py-0.5 rounded border border-white/[0.04] mb-0.5 inline-block ml-1">
                                      {s.start} - {s.end}
                                    </div>
                                  ))}
                                </div>
      ...
      {editingProfessional !== null && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 sm:p-6 lg:p-12 animate-in fade-in duration-300">
          <div className="bg-white/[0.03] rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-500">

            {/* Modal Header */}
            <div className="flex justify-between items-center p-6 border-b border-white/[0.04] bg-white/[0.03] sticky top-0 z-10">
              ...
              <button
                onClick={closeModal}
                className="p-2.5 hover:bg-white/[0.03]/[0.02] rounded-2xl transition-all text-white/30 hover:text-white border border-transparent hover:border-white/[0.04]"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 bg-white/[0.03]/[0.02]/50">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">

                {/* Column 1: Info */}
                <div className="lg:col-span-5 space-y-8">
                  <div className="bg-white/[0.03] p-6 rounded-3xl border border-white/[0.06]/60  space-y-6">
                    <h3 className="text-xs font-black text-white/30 uppercase tracking-[0.2em] flex items-center gap-2 mb-2">
                    ...
                    <div className="space-y-4">
                      ...
                      {editingProfessional.id ? (
                        <div className="group">
                          <label className="block text-[11px] font-black text-white/30 uppercase tracking-wider mb-1.5 ml-1">
                            {t('professionals.clinic_label')}
                          </label>
                          <div className="w-full px-4 py-3 bg-white/[0.03]/[0.04] border border-white/[0.06] rounded-2xl text-sm font-semibold text-white">
                          ...
                          <p className="text-[11px] text-white/30 mt-1 ml-1">{t('professionals.profile_linked_hint')}</p>
                        </div>
                      ) : clinics.length > 0 && (
                        <div className="group">
                          <label className="block text-[11px] font-black text-white/30 uppercase tracking-wider mb-1.5 ml-1 transition-colors group-focus-within:text-primary">
                          ...
                          <select
                            ...
                            className="w-full px-4 py-3 bg-white/[0.03]/[0.02] border border-white/[0.06] rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none text-sm font-semibold text-white appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236b7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.75rem_center] bg-no-repeat"
                          >
                          ...
                          <p className="text-[11px] text-white/30 mt-1 ml-1">{t('professionals.link_professional_hint')}</p>
                        </div>
                      )}
                      ...
                      <div className="group">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 bg-white/[0.03]/[0.02] border border-white/[0.06] rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none text-sm font-semibold text-white"
                  ...
                  <div className="bg-white/[0.03] p-6 rounded-3xl border border-white/[0.06]/60  space-y-6">
                    ...
                    <div className="pt-2 border-t border-white/[0.04]">
                      <label className="flex items-center gap-3 p-4 bg-white/[0.03]/[0.02] rounded-2xl cursor-pointer hover:bg-white/[0.03] border border-transparent hover:border-primary/20 transition-all group">
                        <div className="relative">
                          ...
                          />
                          <div className="w-11 h-6 bg-white/[0.03]/[0.06] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white/[0.03] after:border-white/[0.06] after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"></div>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-white group-hover:text-green-400 transition-colors">{t('professionals.active_staff')}</span>
                          <span className="text-[10px] text-white/30 uppercase font-black">{t('professionals.enabled_global_agenda')}</span>
                        </div>
                      </label>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-1 border-l border-white/[0.04] hidden lg:block"></div>
                ...
                <div className="lg:col-span-6 space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-white/30 font-bold uppercase mt-1 tracking-wider">{t('professionals.define_weekly')}</p>
                    </div>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-3xl border transition-all duration-300 overflow-hidden ${config.enabled
                            ...
                            : 'bg-white/[0.03]/40 border-white/[0.04] grayscale-[0.5]'
                            }`}
                        >
                          <div
                            className={`flex items-center justify-between p-4 cursor-pointer transition-colors ${isExpanded ? 'bg-primary/5' : 'hover:bg-white/[0.03]/[0.02]'
                              }`}
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              >
                                ...
                                />
                                <div className="w-10 h-5 bg-white/[0.03]/[0.06] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white/[0.03] after:border-white/[0.06] after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                              </label>
                              <span className={`text-sm font-black uppercase tracking-widest ${config.enabled ? 'text-white' : 'text-white/30'
                                }`}>
                              ...
                              {config.enabled && (
                                <div className="flex -space-x-1">
                                  {config.slots.map((_, i) => (
                                    <div key={i} className="w-2.5 h-2.5 rounded-full border-2 border-white bg-primary "></div>
                                  ))}
                                </div>
                              )}
                              {config.enabled && (
                            ...
                            <div className="p-5 pt-2 space-y-3 bg-gradient-to-b from-primary/5 to-white animate-in slide-in-from-top-4 duration-300">
                              {config.slots.map((slot, index) => (
                                <div key={index} className="flex items-center gap-3 animate-in fade-in duration-500">
                                  <div className="flex-1 grid grid-cols-2 gap-px bg-white/[0.03]/[0.06] rounded-2xl overflow-hidden  border border-white/[0.04]">
                                    <div className="bg-white/[0.03] p-3 flex flex-col items-center">
                                      <span className="text-[9px] font-black text-white/30 uppercase tracking-[0.15em] mb-1">{t('professionals.entrada')}</span>
                                      <input
                                        type="time"
                                        value={slot.start}
                                    ...
                                    <div className="bg-white/[0.03] p-3 flex flex-col items-center border-l border-white/[0.04]">
                                      <span className="text-[9px] font-black text-white/30 uppercase tracking-[0.15em] mb-1">{t('professionals.salida')}</span>
                                      <input
                                        type="time"
                                        value={slot.end}
                                  ...
                                  <button
                                    ...
                                    className="p-3 text-white/20 hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all border border-transparent hover:border-red-500/20"
                                  >
                                    <X size={16} />
                                  </button>
                                </div>
                              ...
                              <button
                                ...
                                className="w-full py-3 bg-white/[0.03] border-2 border-dashed border-primary/20 rounded-2xl text-[11px] font-black uppercase tracking-widest text-primary/60 hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all "
                              >
            ...
            <div className="p-6 border-t border-white/[0.04] bg-white/[0.03] flex flex-col sm:flex-row justify-end gap-3 sticky bottom-0 z-10 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
              <button
                type="button"
                onClick={closeModal}
                className="w-full sm:w-auto px-8 py-3.5 text-sm font-black uppercase tracking-widest text-white/30 hover:text-white border border-transparent hover:border-white/[0.04] rounded-2xl transition-all min-h-[44px]"
              >
