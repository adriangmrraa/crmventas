...
class Database:
    ...
    async def _run_evolution_pipeline(self, logger):
        ...
        patches = [
            ...
            # Parche 33: DEV-44 — Micro-tareas asignables (created_by, assigned_to, completion_comment)
            """
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='lead_tasks' AND column_name='created_by_user_id') THEN
                    ALTER TABLE lead_tasks ADD COLUMN created_by_user_id UUID REFERENCES users(id) ON DELETE SET NULL;
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='lead_tasks' AND column_name='assigned_to_user_id') THEN
                    ALTER TABLE lead_tasks ADD COLUMN assigned_to_user_id UUID REFERENCES users(id) ON DELETE SET NULL;
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='lead_tasks' AND column_name='completion_comment') THEN
                    ALTER TABLE lead_tasks ADD COLUMN completion_comment TEXT;
                END IF;
                CREATE INDEX IF NOT EXISTS idx_lead_tasks_assigned_to ON lead_tasks (assigned_to_user_id) WHERE assigned_to_user_id IS NOT NULL;
                CREATE INDEX IF NOT EXISTS idx_lead_tasks_created_by ON lead_tasks (created_by_user_id) WHERE created_by_user_id IS NOT NULL;
                RAISE NOTICE 'Parche 33: DEV-44 lead_tasks assignment columns added';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 33: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 34: DEV-47 — Reactivation Sequences
            """
            DO $$ BEGIN
                CREATE TABLE IF NOT EXISTS reactivation_sequences (
                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                    tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                    name TEXT NOT NULL,
                    description TEXT,
                    trigger_after_days INTEGER NOT NULL DEFAULT 7,
                    is_active BOOLEAN DEFAULT TRUE,
                    target_statuses TEXT[] DEFAULT ARRAY['sin_respuesta','seguimiento_pendiente'],
                    created_at TIMESTAMPTZ DEFAULT NOW(),
                    updated_at TIMESTAMPTZ DEFAULT NOW(),
                    CONSTRAINT reactivation_sequences_tenant_name_unique UNIQUE (tenant_id, name)
                );
                CREATE INDEX IF NOT EXISTS idx_reactivation_sequences_tenant ON reactivation_sequences (tenant_id, is_active);

                CREATE TABLE IF NOT EXISTS reactivation_steps (
                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                    sequence_id UUID NOT NULL REFERENCES reactivation_sequences(id) ON DELETE CASCADE,
                    tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                    step_order INTEGER NOT NULL DEFAULT 1,
                    delay_hours INTEGER NOT NULL DEFAULT 24,
                    message_type TEXT NOT NULL DEFAULT 'template' CHECK (message_type IN ('template', 'free_text')),
                    template_id UUID,
                    template_name TEXT,
                ...
                CREATE INDEX IF NOT EXISTS idx_reactivation_steps_sequence ON reactivation_steps (sequence_id, step_order);

                CREATE TABLE IF NOT EXISTS reactivation_logs (
                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                    tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                    lead_id UUID NOT NULL REFERENCES leads(id) ON DELETE CASCADE,
                    sequence_id UUID REFERENCES reactivation_sequences(id) ON DELETE SET NULL,
                    step_id UUID REFERENCES reactivation_steps(id) ON DELETE SET NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_reactivation_logs_tenant_status ON reactivation_logs (tenant_id, status, scheduled_at);
                CREATE INDEX IF NOT EXISTS idx_reactivation_logs_lead ON reactivation_logs (lead_id);

                RAISE NOTICE 'Parche 34: DEV-47 reactivation tables created';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 34: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 35: DEV-49 — Frustration detection columns on leads + chat_conversations
            """
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='leads' AND column_name='frustration_score') THEN
                    ALTER TABLE leads ADD COLUMN frustration_score INTEGER DEFAULT 0;
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='leads' AND column_name='frustration_detected_at') THEN
                    ALTER TABLE leads ADD COLUMN frustration_detected_at TIMESTAMPTZ;
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='leads' AND column_name='frustration_escalated_to') THEN
                    ALTER TABLE leads ADD COLUMN frustration_escalated_to UUID REFERENCES users(id) ON DELETE SET NULL;
                END IF;
                CREATE INDEX IF NOT EXISTS idx_leads_frustration ON leads (tenant_id, frustration_score) WHERE frustration_score > 0;
                RAISE NOTICE 'Parche 35: DEV-49 frustration detection columns added to leads';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 35: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 36: DEV-50 — Duplicate Candidates
            """
            DO $$ BEGIN
                CREATE TABLE IF NOT EXISTS duplicate_candidates (
                    ...
                    created_at TIMESTAMPTZ DEFAULT NOW(),
                    CONSTRAINT duplicate_candidates_pair_unique UNIQUE (tenant_id, lead_a_id, lead_b_id)
                );
                CREATE INDEX IF NOT EXISTS idx_duplicate_candidates_tenant ON duplicate_candidates (tenant_id, status) WHERE status = 'pending';
                CREATE INDEX IF NOT EXISTS idx_duplicate_candidates_leads ON duplicate_candidates (lead_a_id, lead_b_id);
                RAISE NOTICE 'Parche 36: DEV-50 duplicate_candidates table created';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 36: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 37: DEV-55 — Blacklist de seguridad
            """
            DO $$ BEGIN
                CREATE TABLE IF NOT EXISTS blacklist (
                    ...
                    tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                    value TEXT NOT NULL,
                    type TEXT NOT NULL CHECK (type IN ('phone', 'email', 'domain', 'ip')),
                    reason TEXT,
                    created_at TIMESTAMPTZ DEFAULT NOW(),
                    CONSTRAINT blacklist_tenant_value_unique UNIQUE (tenant_id, value)
                );
                CREATE INDEX IF NOT EXISTS idx_blacklist_lookup ON blacklist (tenant_id, value);
                ...
                CREATE INDEX IF NOT EXISTS idx_blacklist_attempts_tenant ON blacklist_attempts (tenant_id, created_at DESC);
                RAISE NOTICE 'Parche 37: DEV-55 blacklist tables created';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 37: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 38: DEV-54 — Full-Text Search index on leads
            """
            DO $$ BEGIN
                CREATE INDEX IF NOT EXISTS idx_leads_search_fts ON leads USING gin(
                    to_tsvector('spanish', 
                        coalesce(first_name, '') || ' ' || 
                        coalesce(last_name, '') || ' ' || 
                        coalesce(email, '') || ' ' || 
                ...
                CREATE INDEX IF NOT EXISTS idx_lead_notes_search_fts ON lead_notes USING gin(
                    to_tsvector('spanish', coalesce(content, ''))
                );

                CREATE INDEX IF NOT EXISTS idx_chat_messages_search_fts ON chat_messages USING gin(
                    to_tsvector('spanish', coalesce(content, ''))
                );
                
                RAISE NOTICE 'Parche 38: DEV-54 FTS indexes created';
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Parche 38: Error: %', SQLERRM;
            END $$;
            """,

            # Parche 39: DEV-51 — Funnel Analytics / Leads conversion tracking columns
            """
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='leads' AND column_name='conversion_value') THEN
                    ALTER TABLE leads ADD COLUMN conversion_value DECIMAL(12,2) DEFAULT 0;
                END IF;
                ...
                END IF;
                RAISE NOTICE 'Parche 39: DEV-51 conversion columns added to leads';
            EXCEPTION WHEN OTHERS THEN
    ...
    async def ensure_lead_exists(
        ...
        # 0. Blacklist check (DEV-55 Security Filter)
        from services.blacklist_service import blacklist_service
        is_blk, reason = await blacklist_service.is_blacklisted(tenant_id, phone_number)
        if not is_blk and _ext_id != phone_number:
            is_blk, reason = await blacklist_service.is_blacklisted(tenant_id, _ext_id)
            
        if is_blk:
            logger.warning(f"🚫 Blocked lead creation for {phone_number}/{_ext_id} (Tenant {tenant_id}): {reason}")
            # Ensure we log the attempt
            await blacklist_service.log_attempt(
                tenant_id, 
                _ext_id, 
                "phone" if _channel == "whatsapp" else "external_id", 
                source, 
                {"phone": phone_number, "name": customer_name, "channel": _channel, "referral": referral}
            )
