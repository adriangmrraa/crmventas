...
const AnalyticsFilters: React.FC<AnalyticsFiltersProps> = ({ onFilterChange }) => {
    ...
    return (
        <div className="bg-white/[0.03] p-4 sm:p-5 rounded-2xl border border-white/[0.04] mb-6">
            <div className="flex flex-wrap gap-4 sm:gap-6 items-end">
                <div className="flex items-center gap-2 min-w-0">
                    <Calendar size={18} className="text-white/40 shrink-0" />
                    <div>
                        <label className="block text-xs font-semibold text-white/50 mb-1 uppercase tracking-wider">{t('analytics.from_date')}</label>
                        <input
                            type="date"
                            value={startDate}
                            onChange={(e) => setStartDate(e.target.value)}
                            className="border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white bg-white/[0.03] focus:outline-none focus:ring-2 focus:ring-medical-500/30 focus:border-medical-500 min-w-[140px]"
                        />
                    </div>
                </div>
                <div>
                    <label className="block text-xs font-semibold text-white/50 mb-1 uppercase tracking-wider">{t('analytics.to_date')}</label>
                    <input
                        type="date"
                        value={endDate}
                ...
                <div className="min-w-[200px] flex-1 sm:flex-initial">
                    ...
                    <select
                        multiple
                        className="border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm w-full min-h-[88px] text-white bg-white/[0.03] focus:outline-none focus:ring-2 focus:ring-medical-500/30 focus:border-medical-500"
